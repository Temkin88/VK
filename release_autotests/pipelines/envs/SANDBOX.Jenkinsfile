...
pipeline {
    ...
    parameters {
        ...
        string(name: 'SANDBOX', defaultValue: 'stage-stable.v3.im-sandbox.devmail.ru', description: 'Sandbox name')
        ...
        string(name: 'DOMAIN_PAID', defaultValue: 'autotest.clients', description: 'Domain of account to get from supply server')
        string(name: 'API_VERSION', defaultValue: '138', description: 'API versions')
        string(name: 'SUPPLY_URL', defaultValue: 'https://imqa-supply.mail.msk/api', description: 'URL используемого supply-сервера')
        ...
        string(name: 'FEDERATION_CONFIG', defaultValue: '', description: 'Federation configuration')
            ...
            steps {
                script {
                    try {
                        ...
                        common.setLaunchName("SANDBOX")
                        common.setLaunchTags("SANDBOX")
            ...
            steps {
                script {
                    try {
                        ...
                        common.loadVarsFromFile('variables.env')
                        common.runPytestApiTests(params.SANDBOX)

                    } catch (Exception error) {
                        println "[E2E API Autotests] $error"
                        STAGE_ERROR="$error"
                        throw error;
                    }
                }
            ...
            steps {
                script {
                    try {
                        ...
                        common.loadVarsFromFile('variables.env')
                        common.runPytestApiTestsLastFailed(params.SANDBOX)

                    } catch (Exception error) {
                        println "[E2E API Autotests - last failed tests] $error"
                        STAGE_ERROR="$error"
                        throw error;
                    }
                }
    ...
    post {
        aborted {
            script {
                def common = load 'pipelines/common.groovy'
                common.reportAbortedBuild("AuCVWqSKY0N_gHtB")
            }
        }
        unsuccessful {
            script {
                def common = load 'pipelines/common.groovy'
                common.reportFailedBuild("AuCVWqSKY0N_gHtB", FAILED_STAGE, STAGE_ERROR)
            }
        }
        success {
            publishHTML (
            ...
            script {
                def common = load 'pipelines/common.groovy'
                common.allureLaunchReport("AuCVWqSKY0N_gHtB")
            }
        }
        always {
            script {
