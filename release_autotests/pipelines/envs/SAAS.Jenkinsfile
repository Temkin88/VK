...
pipeline {
    ...
    parameters {
        ...
        string(name: 'DOMAIN_PAID', defaultValue: '', description: 'Domain paid. example: o-o.bizml.ru')
        string(name: 'SUPPLY_URL', defaultValue: 'https://imqa-supply-prod.mail.msk/api', description: 'URL используемого supply-сервера')
        string(name: 'PYTEST_MARKER', defaultValue: "SAAS", description: 'Markers to select/deselect autotests')
            ...
            steps {
                script {
                    try {
                        ...
                        common.setLaunchName("SAAS (Production)")
                        common.setLaunchTags("SAAS")
