...
pipeline {
    ...
    parameters {
        ...
        string(name: 'PYTEST_MARKER', defaultValue: 'PRE_VKTI', description: 'Markers to select/deselect autotests')
            ...
            steps {
                script {
                    try {
                        ...
                        common.setLaunchName("VKTI (Pre-Production)")
                        common.setLaunchTags("PRE_VKTI")
            ...
            steps {
                script {
                    try {
                        ...
                        common.loadVarsFromFile('variables.env')
                        common.runPytestApiTests('u.internal.myteam.mail.ru')
            ...
            steps {
                script {
                    try {
                        ...
                        common.loadVarsFromFile('variables.env')
                        common.runPytestApiTestsLastFailed('u.internal.myteam.mail.ru')
    ...
    post {
        aborted {
            script {
                def common = load 'pipelines/common.groovy'
                common.reportAbortedBuild("AuHdEIhv1zEYAc8z")
            }
        }
        unsuccessful {
            script {
                def common = load 'pipelines/common.groovy'
                common.reportFailedBuild("AuHdEIhv1zEYAc8z", FAILED_STAGE, STAGE_ERROR)
            }
        }
        success {
            publishHTML (
            ...
            script {
                def common = load 'pipelines/common.groovy'
                common.allureLaunchReport("AuHdEIhv1zEYAc8z")
            }
        }
        always {
            script {
