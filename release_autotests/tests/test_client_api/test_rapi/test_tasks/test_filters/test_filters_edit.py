...
@allure.label("layer", "api_layer")
@allure.title("Редактируем фильтр задач")
@pytest.mark.parametrize(
    ("task_assignees", "is_subscribed", "withChanges", "deadline"),
    [
        (None, False, False, "expired"),
        (None, False, False, "month"),
        (
            "both",
            True,
            True,
...
def test_edit_tasks_filter(
    ...
    with allure.step("Редактируем фильтр"):
        auth_account.tasks_filters_edit(
            filter_id,
            name=filter_name,
            assignees=task_assignees,
            statuses=(task_status,),
            subscribed=not is_subscribed,
            withChanges=not withChanges,
        )
