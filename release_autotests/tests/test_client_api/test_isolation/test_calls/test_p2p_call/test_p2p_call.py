...
def test_p2p_call_isolation(get_voip_bots_isolation):
    """
    P2P-звонок
    """
...
@allure.suite("Звонки")
@allure.feature("P2P-звонки")
@allure.title("p2p звонок Desk + Desk пользователем не в тенанте")
def test_p2p_call_isolation_not_in_tenant(get_voip_bots_isolation):
    """
    P2P-звонок
    """
    with allure.step("Получение VoIP-ботов"):
    ...
    with (
        allure.step("Позвонить с десктопного на десктопный клиент пользователем не в тенанте"),
        pytest.raises(Exception),
    ):
        bot_c.make_p2p_call(to=bot_a, with_video=False)
