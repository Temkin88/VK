...
def test_reactions_get_isolation(
    chat_type,
    reactions_fixt,
    ...
    with allure.step(
        ...
        response = main_acc.rapi_reaction_get(
            chatId=chat,
            msgIds=[msg_id],
        )
        reaction_get_response = response["results"]["reactions"][-1]
        ...
        assert reaction_get_response["myReaction"] == reaction, f"My reaction not matched {reaction}"
