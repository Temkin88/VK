...
def test_basic_forward_isolation(
    chat_type, prepare_public_and_private_test_chats_msg_isolation, third_auth_account_in_tenant
):
    """
    Проверка пересылки обычного текстового сообщения
    """
    main_acc, opponent_acc, chats = prepare_public_and_private_test_chats_msg_isolation
    is_personal_chat, fwd_from, fwd_to = prepare_fwd_from_and_fwd_to_sns(
        opponent_acc, third_auth_account_in_tenant, chats, chat_type
    )
...
@SAAS
@pytest.mark.parametrize("chat_type", ["personal", "group", "channel"])
def test_forward_with_mentions_isolation(
    chat_type, prepare_public_and_private_test_chats_msg_isolation, third_auth_account_in_tenant
):
    """
    ...
    with allure.step("Отправка текстового сообщения с меншенами для последующей пересылки"):
        plain_to_forward_with_mentions = correct_message_with_mentions(
            [opponent_acc.uin, third_auth_account_in_tenant.uin]
        )
    ...
    msg_receiver_acc = third_auth_account_in_tenant if is_personal_chat else opponent_acc
    check_mentions_in_fetch_events(
        mentions_msg_id,
...
@SAAS
@pytest.mark.parametrize("chat_type", ["personal", "group", "channel"])
def test_forward_with_task_creation_isolation(
    chat_type, prepare_public_and_private_test_chats_msg_isolation, third_auth_account_in_tenant
):
    """
    ...
    with allure.step("Проверка пересылки текстового сообщения с дополнительным созданием задачи"):
        check_forwarded_from_info_in_fetch_events(
            account_to_fetch=third_auth_account_in_tenant if is_personal_chat else opponent_acc,
...
@SAAS
@pytest.mark.parametrize("chat_type", ["personal", "group", "channel"])
def test_forward_with_poll_creation_isolation(
    chat_type, prepare_public_and_private_test_chats_msg_isolation, third_auth_account_in_tenant
):
    """
...
@SAAS
@pytest.mark.parametrize("chat_type", ["personal", "group", "channel"])
def test_basic_forward_isolation_not_in_tenant(
    chat_type,
    prepare_public_and_private_test_chats_msg_isolation,
    third_auth_account_in_tenant,
    ...
    with allure.step("Пересылка текстового сообщения пользователем не из тенанта"), pytest.raises(Exception):
        msg_id = first_auth_account_not_in_tenant.forward_message_by_message_send(
            target=fwd_to,
            author_sn=author_sn,
            plain_to_forward=plain_to_forward,
            msg_id=text_msg_id,
