...
@allure.title("Сообщение запрещено при ошибке анализа и strategy_on_fail=block")
@VKTI
@PRE_VKTI
@SAAS
@PRE_SAAS
...
def test_send_message_analysis_fail_and_strategy_on_fail_block(
    chat_type, send_func, chat_entities, check_event_send_message_event_exist
):
    """
    Проверяет, что если DLP не может проанализировать сообщение, но strategy_on_fail=block,
