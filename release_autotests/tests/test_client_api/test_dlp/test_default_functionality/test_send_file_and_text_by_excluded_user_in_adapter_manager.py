...
@allure.title(
    "Логин пользователя добавлен в исключение в adapter_manager. "
    "Отправка файлов с текстом в личку/группу/канал/избранное"
)
@VKTI
@PRE_VKTI
@SAAS
...
def test_send_file_and_text_by_excluded_user_in_adapter_manager(
    chat_type,
    ...
    with allure.step("Отправляем файл получателю"):
        status_code, msg_id = send_func(main_acc, chat, text_to_send, file_url)
