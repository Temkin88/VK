...
@allure.title(
    "Данные на анализ по каждому домену отправляются в соответствующий адаптер. Ответ от DLP по тексту запретить,"
    " система (Вахтёр) должна заблокировать отправку сообщения"
)
@VKTI
@PRE_VKTI
@SAAS
...
def test_upload_file_without_sensitive_data_and_blocked_text_domains_check_true(
