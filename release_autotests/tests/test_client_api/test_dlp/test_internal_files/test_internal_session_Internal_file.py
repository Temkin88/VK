...
@allure.title(
    ...
    " файл Internal. Система (Вахтёр) должна открыть доступ к файлу (чтение/скачивание)"
)
@VKTI
@PRE_VKTI
@SAAS
...
def test_internal_session_internal_file(
    ...
    with allure.step("Оппонент подписывается на трэд"):
        if chat_type == "thread":
