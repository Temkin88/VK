...
@pytest.fixture(scope="session")
def trusted_user_agent_header(dlp_config, config_option):
    if "internal" in dlp_config["adapters"][config_option]["trusted_user_agent"]:
        return dlp_config["adapters"][config_option]["trusted_user_agent"]
    else:
        return None


@pytest.fixture(scope="session")
def untrusted_user_agent_header(dlp_config, config_option):
    if "internal" in dlp_config["adapters"][config_option]["trusted_user_agent"]:
        return dlp_config["adapters"][config_option]["trusted_user_agent"]
    else:
        return None
...
@pytest.fixture(scope="session")
def main_acc(account_fabric, dlp_config, config_option, trusted_user_agent_header, default_main_acc):
    ...
    header = {"User-Agent": trusted_user_agent_header} if trusted_user_agent_header is not None else {}
...
@pytest.fixture(scope="session")
def main_acc_second_instance(account_fabric, main_acc, untrusted_user_agent_header, default_main_acc_second_instance):
    ...
    header = {"User-Agent": untrusted_user_agent_header} if untrusted_user_agent_header is not None else {}
    return account_fabric(
        header=header,
...
@pytest.fixture(scope="session")
def trusted_subnets(dlp_config, config_option) -> list[str]:
    if "trusted_subnets" not in dlp_config["adapters"][config_option]:
        pytest.skip("Trusted subnets not described")
    subnets = dlp_config["adapters"][config_option]["trusted_subnets"]
    if len(subnets) == 0:
        pytest.skip("Blocking by subnets disabled")
    return subnets
