...
from tests.test_client_api.test_dlp.test_whitelisting.common import check_user_ip_addr_in_trusted_subnets
...
@allure.title(
    "Загрузка файла c из клиента с разрешенной/запрещенной КСПД. Отправка файла в личку/группу/канал/избранное."
)
@VKTI
@PRE_VKTI
@SAAS
...
def test_untrusted_by_subnet_users_blocking(
    trusted_subnets,
    chat_type,
    ...
    for friendly_name, acc in accounts.items():
        with allure.step(f"{friendly_name} пытается получить доступ к файлу"):
            if check_user_ip_addr_in_trusted_subnets(acc, trusted_subnets):
