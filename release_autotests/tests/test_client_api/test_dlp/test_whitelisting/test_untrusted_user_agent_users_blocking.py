...
@allure.title(
    "Загрузка файла c из клиента с разрешенной/запрещенной по USER-AGENT КСПД."
    " Отправка файла в личку/группу/канал/избранное."
)
@VKTI
@PRE_VKTI
@SAAS
...
def test_untrusted_user_agent_users_blocking(
    trusted_user_agent_header,
    untrusted_user_agent_header,
    chat_type,
    ...
    for friendly_name, acc in accounts.items():
        with allure.step(f"{friendly_name} пытается получить доступ к файлу"):
            if acc.session.headers.get("User-Agent") == trusted_user_agent_header:
