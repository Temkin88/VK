...
class AuthorizationOracle(abc.ABC):
    ...
    @abc.abstractmethod
    def is_unauthorized_request(self, flow: mock_server.mitmim.mitmproxy.http.HTTPFlow) -> bool:
        pass

    @abc.abstractmethod
    def is_unauthorized_response(self, flow: mock_server.mitmim.mitmproxy.http.HTTPFlow) -> bool:
        pass
...
class SeleniumAddon:
    ...
    *login*. In order to use the add-on, a class for the web application inheriting from SeleniumAddon needs to be
    ...
    def request(self, flow: mock_server.mitmim.mitmproxy.http.HTTPFlow):
    ...
    @concurrent
    def response(self, flow: mock_server.mitmim.mitmproxy.http.HTTPFlow):
    ...
    def _set_set_cookie_headers(self, flow: mock_server.mitmim.mitmproxy.http.HTTPFlow):
    ...
    def _set_request_cookies(self, flow: mock_server.mitmim.mitmproxy.http.HTTPFlow):
        if self.cookies:
            cookies = "; ".join(
    ...
    @abc.abstractmethod
    def login(self, flow: mock_server.mitmim.mitmproxy.http.HTTPFlow) -> List[Dict[str, str]]:
        pass
