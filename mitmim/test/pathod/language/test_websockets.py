...
from mock_server.mitmim.mitmproxy.pathod.language import websockets
...
class TestWebsocketFrame:
    ...
    def test_flags(self):
        ...
        frm = mock_server.mitmim.mitmproxy.net.websockets.Frame.from_bytes(tservers.render(wf))
    ...
    def fr(self, spec, **kwargs):
        ...
        return mock_server.mitmim.mitmproxy.net.websockets.Frame.from_bytes(tservers.render(wf, settings))

    def test_construction(self):
        assert self.fr("wf:c1").header.opcode == 1
        assert self.fr("wf:c0").header.opcode == 0
        assert self.fr("wf:cbinary").header.opcode ==\
            mock_server.mitmim.mitmproxy.net.websockets.OPCODE.BINARY
        assert self.fr("wf:ctext").header.opcode ==\
            mock_server.mitmim.mitmproxy.net.websockets.OPCODE.TEXT
