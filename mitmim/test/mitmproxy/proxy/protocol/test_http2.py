...
import mock_server.mitmim.mitmproxy.net
...
from mock_server.mitmim.mitmproxy.net.http import http1, http2
...
class _Http2ServerBase(net_tservers.ServerTestBase):
    ...
    class handler(mock_server.mitmim.mitmproxy.net.tcp.BaseHandler):
...
class _Http2TestBase:
    ...
    def setup_connection(self):
        self.client = mock_server.mitmim.mitmproxy.net.tcp.TCPClient(("127.0.0.1", self.proxy.port))
        self.client.connect()

        # send CONNECT request
        self.client.wfile.write(http1.assemble_request(
            mock_server.mitmim.mitmproxy.http.make_connect_request(("localhost", self.server.server.address[1]))
        ))
