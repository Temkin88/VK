...
def decode_zstd(content: bytes) -> bytes:
    if not content:
        return b""
    global response_dictionary
    if response_dictionary is None:
        with open(os.path.join(os.path.dirname(__file__), "im-zstd-dict-desktop-response-210720.zdict"), 'rb') as f:
            response_dictionary = f.read()
        f.close()
    dict_data = zstd.ZstdCompressionDict(response_dictionary)
    zstd_ctx = zstd.ZstdDecompressor(dict_data=dict_data)
    try:
        return zstd_ctx.decompress(content)
    except Exception as e:
        global request_dictionary
        if request_dictionary is None:
            with open(os.path.join(os.path.dirname(__file__), "im-zstd-dict-desktop-request-210720.zdict"), 'rb') as f:
                request_dictionary = f.read()
            f.close()
        dict_data = zstd.ZstdCompressionDict(request_dictionary)
        zstd_ctx = zstd.ZstdDecompressor(dict_data=dict_data)
        return zstd_ctx.decompress(content)
