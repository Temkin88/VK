...
importmock_server.mitmim.mitmproxylow
from mock_server.mitmim.mitmproxy import flowfilter
from mock_server.mitmim.mitmproxy import exceptions
...
from mock_server.mitmim.mitmproxy.utils import human
...
class _OrderKey:
    ...
    def generate(self, f: mock_server.mitmim.mitmproxy.flow.Flow) -> typing.Any:  # pragma: no cover
        pass
...
class OrderRequestStart(_OrderKey):
    def generate(self, f: mock_server.mitmim.mitmproxy.flow.Flow) -> float:
        return f.timestamp_start


class OrderRequestMethod(_OrderKey):
    def generate(self, f: mock_server.mitmim.mitmproxy.flow.Flow) -> str:
...
class OrderKeySize(_OrderKey):
    def generate(self, f: mock_server.mitmim.mitmproxy.flow.Flow) -> int:
        if isinstance(f, http.HTTPFlow):
            size = 0
...
class View(collections.abc.Sequence):
    ...
    def _bisect(self, f:mock_server.mitmim.mitmproxylow.Flow) -> int:
        v = self._view.bisect_right(f)
        return self._rev(v - 1) + 1

    def index(self, f:mock_server.mitmim.mitmproxylow.Flow, start: int = 0, stop: typing.Optional[int] = None) -> int:
    ...
    def getvalue(self, flow:mock_server.mitmim.mitmproxylow.Flow, key: str, default: str) -> str:
        """
    ...
    def setvalue(
        self,
        flows: typing.Sequence[mock_server.mitmim.mitmproxy.flow.Flow],
        key: str, value: str
    ) -> None:
        """
    ...
    def load_file(self, path:mock_server.mitmim.mitmproxyypes.Path) -> None:
        """
...
class Settings(collections.abc.Mapping):
    ...
    def __getitem__(self, f:mock_server.mitmim.mitmproxylow.Flow) -> dict:
