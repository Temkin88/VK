...
from mock_server.mitmim.mitmproxy import exceptions, command_lexer
from mock_server.mitmim.mitmproxy.command_lexer import unquote
...
def typename(t: type) -> str:
    ...
    to =mock_server.mitmim.mitmproxyypes.CommandTypes.get(t, None)
    if not to:
...
class Command:
    ...
    def __init__(self, manager: "CommandManager", name: str, func: typing.Callable) -> None:
        ...
        for name, parameter in self.signature.parameters.items():
            ...
            if not mock_server.mitmim.mitmproxy.types.CommandTypes.get(parameter.annotation, None):
                raise exceptions.CommandError(f"Argument {name} has an unknown type ({_empty_as_none(t)}) in {func}.")
        if self.return_type and not mock_server.mitmim.mitmproxy.types.CommandTypes.get(self.return_type, None):
    ...
    def call(self, args: typing.Sequence[str]) -> typing.Any:
        ...
        if ret is None and self.return_type is None:
            return
        typ =mock_server.mitmim.mitmproxyypes.CommandTypes.get(self.return_type)
        assert typ
...
class CommandManager:
    ...
    def parse_partial(
        ...
        next_params: typing.List[CommandParameter] = [
            CommandParameter("",mock_server.mitmim.mitmproxyypes.Cmd),
            CommandParameter("",mock_server.mitmim.mitmproxyypes.CmdArgs),
        ]
        ...
        for part in parts:
            ...
            if expected and expected.kind is inspect.Parameter.VAR_POSITIONAL:
                ...
                expected = CommandParameter("",mock_server.mitmim.mitmproxyypes.Unknown)

            arg_is_known_command = (
                    expected.type ==mock_server.mitmim.mitmproxyypes.Cmd and part in self.commands
            )
            arg_is_unknown_command = (
                    expected.type ==mock_server.mitmim.mitmproxyypes.Cmd and part not in self.commands
            )
            command_args_following = (
                    next_params and next_params[0].type ==mock_server.mitmim.mitmproxyypes.CmdArgs
            )
            ...
            to =mock_server.mitmim.mitmproxyypes.CommandTypes.get(expected.type, None)
            valid = False
            if to:
                try:
    ...
    def execute(self, cmdstr: str) -> typing.Any:
        ...
        command_name, *args = [
            unquote(part.value)
            for part in parts
            if part.type !=mock_server.mitmim.mitmproxyypes.Space
        ]
...
def parsearg(manager: CommandManager, spec: str, argtype: type) -> typing.Any:
    ...
    """
    t =mock_server.mitmim.mitmproxyypes.CommandTypes.get(argtype, None)
    if not t:
...
def argument(name, type):
    """
        ...
        specific types such asmock_server.mitmim.mitmproxyypes.Choice, which we cannot annotate
