...
importmock_server.mitmim.mitmproxyommand
importmock_server.mitmim.mitmproxylow
importmock_server.mitmim.mitmproxyaster
importmock_server.mitmim.mitmproxyypes
...
class CommandBuffer:
    def __init__(self, master:mock_server.mitmim.mitmproxyaster.Master, start: str = "") -> None:
        self.master = master
        self.text = start
    ...
    def render(self):
        ...
        if not parts:
            ...
            for p in parts:
                if p.valid:
                    if p.type ==mock_server.mitmim.mitmproxyypes.Cmd:
            ...
            if remaining:
                if parts[-1].type !=mock_server.mitmim.mitmproxyypes.Space:
                    ret.append(("text", " "))
    ...
    def cycle_completion(self, forward: bool = True) -> None:
        if not self.completion:
            ...
            if parts and parts[-1].type !=mock_server.mitmim.mitmproxyypes.Space:
                ...
                return
            ct =mock_server.mitmim.mitmproxyypes.CommandTypes.get(type_to_complete, None)
            if ct:
...
class CommandEdit(urwid.WidgetWrap):
    ...
    def __init__(self, master:mock_server.mitmim.mitmproxyaster.Master, text: str) -> None:
