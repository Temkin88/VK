...
from mock_server.mitmim.mitmproxy import connections, flow  # noqa
from mock_server.mitmim.mitmproxy import exceptions
from mock_server.mitmim.mitmproxy import http
from mock_server.mitmim.mitmproxy.proxy.protocol import base
from mock_server.mitmim.mitmproxy.proxy.protocol import http as httpbase
import mock_server.mitmim.mitmproxy.net.http
from mock_server.mitmim.mitmproxy.net import tcp
from mock_server.mitmim.mitmproxy.coretypes import basethread
from mock_server.mitmim.mitmproxy.net.http import http2, headers, url
...
class Http2Layer(base.Layer):
    ...
    def _handle_request_received(self, eid, event):
        headers = mock_server.mitmim.mitmproxy.net.http.Headers([[k, v] for k, v in event.headers])
    ...
    def _handle_pushed_stream_received(self, event):
        ...
        headers =mock_server.mitmim.mitmproxyet.http.Headers([[k, v] for k, v in event.headers])
...
class Http2SingleStreamLayer(httpbase._HttpTransmissionLayer, basethread.BaseThread):
    ...
    class Message:
        def __init__(self, headers=None):
            self.headers: Optional[mock_server.mitmim.mitmproxy.net.http.Headers] = headers  # headers are the first thing to be received on a new stream
    ...
    def __init__(self, ctx, h2_connection, stream_id: int, request_headers: mock_server.mitmim.mitmproxy.net.http.Headers) -> None:
        super().__init__(
