...
@pytest.fixture(params=[".", "sw-report", "sw-report/subdir"])
def clean_report_folders(request):
    yield request.param

    report_folder_path = pathlib.Path(request.param)

    report_folder_path.joinpath("spy-report-vk_teams.botapi.html").unlink()
...
def test_create_report(clean_report_folders, clear_db):
    spy = http_spy.SwaggerHttpSpy(
        ...
        exclude_json="./test-exclude.json",
    )
    ...
    assert report_folder.joinpath(report_filename).exists(), (
        f'Report "{report_filename}" does not exist in {spy.report_path}'
    )


def test_raise_report_on_none_path_value(clear_db):
    spy = http_spy.SwaggerHttpSpy(
        service_name="vk_teams.botapi",
        targets=["https://myteam.mail.ru/botapi/api.yaml?4624464"],
        api_prefix="/bot/v1",
...
def test_report_regexp_path(swagger, session, clear_db):
...
def test_report_integrity_error(swagger, clear_db):
    ReportInfo.create(**{"info_type": "service", "info_text": "self.service"})
