...
class AuthMethods(BaseClient):
    """
    Методы группы 'Авторизация' из https://im-docs.corp.mail.ru/
    """

    @tracer.start_as_current_span("auth.rapi_auth_guest")
    @api_version_check(min_version=92)
    def rapi_auth_guest(self, name: str, scopes: list[str]):
        return self.post(
            path="rapi/auth/guest",
    ...
    @api_version_check(min_version=87)
    @tracer.start_as_current_span("auth.rapi_auth_oidc_authorize")
    ...
    @api_version_check(min_version=87)
    @tracer.start_as_current_span("auth.rapi_auth_oidc_submitCode")
    def rapi_auth_oidc_submitCode(self, state: str, code: str) -> dict:
        ...
        :return: тело ответа сервера
        """
        return self.get(path="rapi/auth/oidc/submitCode", state=state, code=code)

    @api_version_check(min_version=1)
    @tracer.start_as_current_span("auth.rapi_auth_is_strong_password")
    ...
    @tracer.start_as_current_span("auth.rapi_auth_webapp_session_start")
    ...
    @api_version_check(min_version=1)
    @tracer.start_as_current_span("auth.rapi_auth_send_сode")
    ...
    @api_version_check(min_version=1)
    @tracer.start_as_current_span("auth.rapi_auth_sign_in")
    ...
    @api_version_check(min_version=1)
    @tracer.start_as_current_span("auth.rapi_auth_sign_in_with_password")
    ...
    @api_version_check(min_version=1)
    @tracer.start_as_current_span("auth.rapi_auth_attach_phone")
    ...
    @api_version_check(min_version=1)
    @tracer.start_as_current_span("auth.rapi_auth_reset_password")
    ...
    @api_version_check(min_version=37)
    @tracer.start_as_current_span("auth.rapi_auth_appToken")
    ...
    @api_version_check(min_version=114)
    @tracer.start_as_current_span("auth.rapi_auth_refreshImserverTokens")
